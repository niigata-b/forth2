	...
	<form action="SubmitUpdateEmployeeServlet" method="post">
		...
		<p>
			性別: <select name="gender">
				<option value="男"
					<%= "男".equals(emp.getGender()) ? "selected" : "" %>>男</option>
				<option value="女"
					<%= "女".equals(emp.getGender()) ? "selected" : "" %>>女</option>
				<option value="その他"
		...
		<p>
			年齢: <select name="age">
				<% for (int i = 1; i <= 100; i++) { %>
				<option value="<%= i %>"
					<%= (i == emp.getAge()) ? "selected" : "" %>><%= i %></option>
				<% } %>
			</select>
		</p>
		<p>
